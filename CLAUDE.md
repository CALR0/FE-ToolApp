...
>
> **✅ PROBADO end-to-end (2026-08-29)** con el 25/08 (3 facturas, 35 remesas) y el **26/08 (118 facturas, 119 remesas)**. Login OK, datos OK (NIT+dígito, CUFE 96, radicados resueltos), XML generado y **enviado al RNDC de verdad**. Resultado (esperado, ya estaban cargadas): FAC038 duplicado → el XML es **estructuralmente válido**. Notas: `PLAYWRIGHT_HEADLESS=1` y forzar `sys.stdout.reconfigure(utf-8)` (consola Windows cp1252 rompe con ✓/✗).
> **🐛 BUG encontrado y corregido en la prueba del 26/08 — PAGINACIÓN**: `GetDocumentsDescargaMasiva` con `pageSize=100` solo traía la 1ª página (el 26 tenía 118 → se perdían 18). `facture_client.listar_facturas` ahora **recorre todas las páginas** (`pageIndex` 1..N) hasta traer todo, con parada robusta que NO depende solo de `totalItemCount` (para si: página incompleta = última, o `len>=total`, o página sin nada nuevo), deduplica por LDF/número, tope 100k. **`pageSize` máx = 100** (200 da HTTP 400). Probado: 118 en 5 páginas (page_size 25) y rango de mes (153) → sin pérdidas. **Split de perfil por factura verificado** (25 y 26 fueron todas TSP; falta una corrida con Elogia para verlo en vivo).
>
...
> **Reporte final (2026-08-29):** `plcolab-rpa/reporte.py` genera al terminar el pipeline:
> - **Excel** `Facturas PLCOLAB <DD-MM-YYYY>.xlsx` (fecha = emisión/consulta) con el formato del ejemplo del usuario (`Reporte facturas ...xlsx`): columnas **FECHA DE EMISION · FECHA ENVIO · CLIENTE · FACTURA UT · CANTIDAD DE REMESAS · REMESAS ASOCIADAS · ¿Subió al RNDC? · Novedad**. FECHA DE EMISION = fecha de consulta; FECHA ENVIO = día en que el bot lo corrió. Encabezado azul (accent5 `#5B9BD5`) blanco bold. `¿Subió?` = SI/NO; `Novedad` = error del RNDC, o "Factura electronica grabada con éxito. Radicado: <ingresoid>" si sube (ingresoid extraído de "Radicado RNDC: <n>").
>   Encabezados en la **fila 1** (freeze panes `A2`), FACTURA UT como número, y al final un bloque de **totales**: TOTAL DE FACTURAS, TOTAL DE REMESAS, TOTAL SUBIDAS (SI), TOTAL NO SUBIDAS (NO).
>   - **Novedad detallada para FAC080** (`fetool_bridge.remesas_sin_cumplir` + `remesas_pendientes_manifiesto` + `pipeline`): cuando el RNDC rechaza por "remesa sin cumplir", la Novedad lista los consecutivos NO cumplidos → `Remesas sin cumplir: <consec…>`, y si alguna además está **sin manifiesto** agrega ` · Pendientes de asignar manifiesto: <consec…>`. Se identifica por **`estado != 'CE'`** (CE=cumplida; AC=pendiente) + `manifiesto` vacío, usando el `estado`/`manifiesto` que ya devuelve `consultar_radicado_remesa` (sin consultas extra). Ojo: una remesa AC puede tener manifiesto asignado pero sin cumplir (por eso el criterio base es `!= CE`, no "AC sin manifiesto").
> - **Zip** `Facturas PLCOLAB <DD-MM-YYYY>.zip` con todos los XML generados.
> - **Auditoría**: `datos_rg PLCOLAB <DD-MM-YYYY>.xlsx` (`reporte.guardar_datos_rg`, `df.to_excel`) — el datos_rg físico (una fila por remesa) que alimenta la generación, equivalente al del flujo manual. Se guarda en `salidas/` (va al artefacto de GitHub).
> - Se guardan en `plcolab-rpa/salidas/` (requiere `openpyxl`). **Entrega: (1) CORREO** (`enviar.py`, SMTP/Gmail app-password vía `SMTP_*`/`MAIL_TO` en `.env`/Secrets; si no está configurado se omite) **+ (2) ARTEFACTO de GitHub Actions** (`salidas/` en el workflow, respaldo sin credenciales).
>   - **Correo HTML cordial** (`enviar.py`: `_shell`, `html_con_facturas`, `html_sin_facturas`), dos casos: **con facturas** → saludo + "el día X se encontraron N facturas" + tabla de totales + Excel y zip adjuntos; **sin facturas** → correo cordial de "no se encontró información para el día X" (el pipeline lo envía incluso cuando el listado viene vacío, antes del return). Asunto `Facturas PLCOLAB EMITIDAS <fecha emisión>` (o `… — Sin facturas`). `MAIL_TO` admite varios separados por coma. **Envío verificado 2026-08-29** (Gmail, a carandres203@gmail.com).
...
3. ✅ **Confirmado: `Content3` trae TODO lo que hoy saca el PDF** → la automatización arma el `datos_rg` **directo del JSON, sin descargar ni parsear PDF** (`lib_extraer` deja de ser necesario en el camino API). **Verificado sobre factura 411930 (12 remesas): las 12 líneas traen consecutivo, y Σ de valores de línea = `amount` exacto (28.789.948).** Mapeo columna RG → ruta en `Content3`/listado:
   ...
   - `consecutivo_remesa` → **SELECCIÓN ROBUSTA entre `"02"` y `CodigoItem`** (ver `facture_client.consecutivo_de_linea`). Facture es **inconsistente**: mete el valor o el radicado en uno de esos dos campos, y NINGUNO es 100% fiable por sí solo:
     - 411930: `CodigoItem` traía el **radicado** (`163661861`) y `"02"` el consecutivo (`30013623`).
     - 412057 (2026-09-09): `"02"` traía el **valor** (`1736264`, `1381809`) y `CodigoItem` el consecutivo (`30014312`, `30014315`).
     Regla: entre `"02"` y `CodigoItem`, tomar el que tenga **forma de consecutivo** (regex `^(300|120|0101|101)\d`, = prefijo de perfil) y que **NO** sea el valor de la línea. Si ninguno califica → queda como "perfil no resuelto" (no se genera XML, se marca en el reporte). `ConsecutivoItem`=índice; `ValorTotalItem`(=prop `"03"`)=valor.
