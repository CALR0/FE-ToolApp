...
| `consultar_monitoreo_manifiesto(perfil, radicado_manifiesto="", placa="", timeout=20)` | `services/rndc_service.py` | **Proceso 60** / `tipo=3` / `variables=*` — **tiempos logísticos (monitoreo)**. Filtra por `INGRESOIDMANIFIESTO` (radicado) y/o `NUMPLACA`; hay que pasar al menos uno. Usa credenciales de monitoreo (`rndc_usuario_monitoreo`, fallback normales), `NUMIDGPS=nit_monitoreo`, `NUMNITEMPRESATRANSPORTE=nit_socio`. Retorna `(ok, list[dict])` con **un documento por punto de control** |
